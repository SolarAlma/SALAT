...
def ALMA_fits_reader(path_alma_fba,Shahin_format=False):
    ...
    Parameters
    ----------
    path_alma: path to fits files
    Shahin_format: if True, input path_alma with *.fits 
                    if False, input path_alma as directory path containing .fits-Default SoAP

    Returns
    -------
    dfba_alma: cropped data cube with snapshots as a numpy array
    tfba_alma: numpy array with times as datetime format
    arpx_alma: float, pixel size
