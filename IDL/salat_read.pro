...
function salat_read, cube, header=header, time=time, beam_major=beam_major, beam_minor=beam_minor, $
                     beam_angle=beam_angle, nan_value=nan_value, nan_median=nan_median, silent=silent, no_header=no_header

alma = reform(readfits(cube,header,/silent))
variables = reform(readfits(cube,header_var,ext=1,/silent))
soap_parameters = reform(readfits(cube,header_soap,ext=2,/silent))
...
beammajor = reform(variables[*,0])
beamminor = reform(variables[*,1])
tbeamangleime = reform(variables[*,2])
