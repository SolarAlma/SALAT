PRO sjclock, time, pos=pos, size=siz, fill=fill, dev=dev, thick=thick, col=col
;+
; NAME:
;       CLOCK
; PURPOSE:
...
if n_elements(col) eq 0 then col=cgColor('Gold') else col=col
;loadct, 0
;col = 0
...
IF keyword_set(fill) THEN $
  ...
  [pos(1), pos(1)+siz(1), pos(1)+siz(1), pos(1)], /norm, col = col


FOR i = 0, 330, 30 DO BEGIN
    phi = i*!Pi/180
    plots, [ri, ra]*cos(phi)*siz(0)/2+cx, $
           [ri, ra]*sin(phi)*siz(1)/2+cy, /norm, thick=thick, color=col
ENDFOR

phi = (h+m/60.)*3*!pi/18
sp = sin(phi) & cp = cos(phi)
polyfill, 0.7*x*[cp, sp, cp, sp, cp]*siz(0)/2+cx, $
          0.7*y*[sp, cp, sp, cp, sp]*siz(1)/2+cy, /norm, color=col


phi = (m*6.+s/10.) * !Pi/180
sp = sin(phi) & cp = cos(phi)
polyfill, x*[cp, sp, cp, sp, cp]*siz(0)/2+cx, $
          y*[sp, cp, sp, cp, sp]*siz(1)/2+cy, /norm, color=col
